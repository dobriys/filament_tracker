...
1. **Настройки → Диагностический журнал** → включите **«Записывать действия и
   ошибки»** (доступно администратору).
...
Что попадает в журнал (пока он включён): действия пользователя как запросы к
серверу (метод, путь, код ответа, длительность, тело — без секретов) и текст
ошибки при неуспехе; фоновая автоматизация принтера (импорт печатей,
автосписание и причины, почему оно не сработало, опрос принтеров); необработанные
ошибки сервера с трассировкой; ошибки в браузере. Пароли, ключи и токены в теле
запросов **вырезаются автоматически**. Журнал хранится в вашей базе (последние
5000 записей, старые вытесняются) — во внешние сервисы ничего не отправляется.
Личных данных в записях, как правило, нет, но перед публикацией просмотрите файл.
