...
- **Уведомления в Telegram** — печать завершена, ошибка, пауза, сушка, катушка
  заканчивается. Каждое событие включается отдельно, а к событиям печати можно
  приложить кадр с камеры принтера.
...
- **Расчёт стоимости печати** — полная себестоимость изделия: филамент, электричество,
  износ принтера, ваше время, комплектующие и упаковка. И цена, которая оставляет
  нужную наценку.
- **Тарифы у самого принтера** — мощность, цена, срок службы и загрузка задаются
  на его карточке, там же видно, во сколько выходит час печати. Отдельного экрана
  настроек нет: цифры лежат там, к чему относятся.
- **Расчёт из готовой печати** — граммы и время подставляются из истории, а цена
  филамента выводится из того, что реально списано с катушек.
- **Своя валюта** — рубли, доллары, евро и ещё пять валют. Подписи полей и суммы
  следуют за выбором, а сохранённый расчёт остаётся в той валюте, в которой посчитан.
...
Печати, которые ещё не списаны (черновики), списываются прямо отсюда — кнопкой в
строке, даже если задание давно уехало из истории принтера.
...
Сколько на самом деле стоит напечатать вещь и почём её продавать. Себестоимость
разложена на материалы, вашу работу, износ принтера и упаковку; тарифы у сохранённого
расчёта замораживаются, чтобы отправленная заказчику цена не менялась задним числом.
Граммы и время можно взять из готовой печати, а цена филамента тогда посчитается по
фактически списанным катушкам. У непонятных полей — знак вопроса с объяснением.

![Расчёт стоимости печати](docs/screenshots/cost.png)

Во сколько обходится час работы, задаётся у самого принтера: цена, апгрейды,
обслуживание, срок службы, загрузка, мощность и тариф на электричество. Пустое
поле — значение по умолчанию, оно же стоит в подсказке.

![Тарифы принтера](docs/screenshots/printer-cost.png)
...
Рядом со списком событий выбирается **часовой пояс**. Он нужен только сообщениям:
в браузере время приводится к поясу устройства само, а текст уведомления собирает
сервер, и без этой настройки он взял бы пояс контейнера — обычно UTC. Если в
сообщении «Завершена» стоит время на несколько часов раньше вашего, укажите пояс
(в списке уже подставлен пояс вашего устройства).
...
К событиям печати прикладывается снимок с камеры принтера: сначала картинка, под
ней текст уведомления. Включает это отдельный выключатель **«Прикладывать кадр с
камеры принтера»**, а галочки рядом выбирают события — началась, завершена,
ошибка, отменена, пауза.

Адрес снимка приложение выясняет само: спрашивает у Moonraker список камер
(`/server/webcams/list`), а если тот молчит, пробует типовой путь
`/webcam/?action=snapshot` — и на порту Moonraker, и на 80-м, где камеру обычно
раздаёт nginx. Найденный адрес запоминается, чтобы не искать заново каждый раз.
Свой адрес задаётся в разделе «Принтеры» → **Moonraker** → **Камера принтера**;
там же кнопка **«Сохранить и проверить»** показывает, что видит камера. Годится и
поток `?action=stream` — из него берётся первый кадр.

Камера не ответила — уведомление всё равно придёт, просто без картинки.
