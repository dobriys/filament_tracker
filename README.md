...
> 🕹️ **[Живое демо →](https://demo.fmtracker.ru)** — потрогать интерфейс на
> готовых данных без установки. Всё работает прямо в браузере (бэкенд не нужен),
> правки хранятся локально, кнопка «Сбросить демо» возвращает исходные данные.
...
- 🗂️ **Каталог принтеров** — выбор модели из 50+ совместимых Klipper/Moonraker-принтеров
  (Anycubic, Creality, Sovol, QIDI, FLSUN, ELEGOO, Prusa и др.).
...
- 📨 **Уведомления в Telegram** — печать завершена, ошибка, пауза, сушка, катушка
  заканчивается. Каждый тип событий включается отдельно.
- 🌡️ **Датчики из Home Assistant** — температура и влажность в сушилке и в местах
  хранения на панели; выше порога — подсветка и уведомление.
- 📄 **Расчёт по gcode** — загрузите файл, приложение посчитает расход по каждой
  печати и спишет с нужных катушек.
...
Количество принтеров неограничено. 
...
расход, а вы сопоставите каждый инструмент с катушкой
...
- **Уведомления в Telegram** — сообщения об изменениях состояния принтера и склада.
  Создайте бота через [@BotFather](https://t.me/BotFather), вставьте токен, отправьте
  своему боту «/start» и нажмите «Определить chat id» — поле заполнится само.
  Затем «Отправить тест» и галочками отметьте, что именно вам нужно:

  | Событие | По умолчанию |
  | --- | --- |
  | Печать завершена | ✅ |
  | Ошибка печати (с текстом от прошивки) | ✅ |
  | Печать на паузе | ✅ |
  | Печать началась | — |
  | Печать отменена | — |
  | Сушка включена | — |
  | Сушка завершена | ✅ |
  | Принтер недоступен / снова на связи | — |
  | Автосписание не выполнено | ✅ |
  | Катушка заканчивается (порог настраивается) | ✅ |
  | Влажность выше порога (датчик Home Assistant) | ✅ |

  Состояние принтера отслеживает фоновый поллер, поэтому уведомления приходят и
  когда приложение не открыто в браузере. Токен бота хранится в зашифрованном виде
  и наружу не отдаётся. Пока общий выключатель выключен, принтеры ради уведомлений
  не опрашиваются.
- **Датчики температуры и влажности (Home Assistant)** — если дома есть Home
  Assistant, приложение показывает показания ваших датчиков там, где они нужны:
  на панели, под карточкой принтера и в списке мест хранения. Укажите адрес HA
  (например `http://homeassistant.local:8123`) и токен долгосрочного доступа (профиль в
  HA → «Токены долгосрочного доступа»), затем «Загрузить список датчиков» — поля
  сущностей начнут подсказывать варианты. Источник датчика неважен: zigbee2mqtt,
  ESPHome, Bluetooth — приложение берёт готовое состояние из HA.

  Каждый датчик привязывается к принтеру (показания встают рядом с сушилкой),
  к месту хранения (видно в списке мест) или показывается отдельным блоком на
  панели. Влажность выше порога подсвечивается и, если включено уведомление,
  уходит в Telegram. Токен хранится в зашифрованном виде; пока общий
  выключатель выключен, Home Assistant не опрашивается.

  Порог влажности задаётся у каждого датчика отдельно, а общий из настроек
  работает как значение по умолчанию. Это важно, потому что одно число на всех
  не годится: в горячей сушилке нагрев сам по себе занижает относительную
  влажность (28 % при 46 °C по абсолютному содержанию влаги «мокрее», чем 50 %
  при 22 °C), а нейлону и PVA нужен куда более жёсткий порог, чем PLA. Ориентиры:
  ...
  Учтите точность бытовых датчиков (обычно ±3–5 % RH) — слишком узкие пороги
  смысла не имеют.
- **Списание** — разрешить уход остатка в минус (только администратор).
- **Диагностический журнал** — включаемая запись действий и ошибок для отладки:
  при проблеме включите запись, повторите действия, скачайте журнал и приложите
  его к issue. См. [Как сообщить об ошибке](#как-сообщить-об-ошибке).
...
7. Печатайте — и списывайте расход с главной страницы в один клик или автоматически.
...
Если что-то работает не так, приложите к
[issue на GitHub](https://github.com/dobriys/filament_tracker/issues) журнал
диагностики — так проблему видно и её проще исправить:

1. **Настройки → Диагностический журнал** → включите **«Записывать действия и
   ошибки»**.
2. Повторите действия, на которых возникает проблема.
3. Нажмите **«Скачать (.txt)»** и приложите файл к issue (или отфильтруйте записи
   на месте и раскройте «подробнее»). Опишите, что делали и что ожидали увидеть.
...
Оценка остатка появляется после ~2% прогресса. Она считается по прошедшему времени
и прогрессу, а при почти нулевом прогрессе результат недостоверен (могут быть
«сотни часов»), поэтому в самом начале печати он скрыт. Как только прогресс
перевалит порог — остаток появится сам.
...
Если этот проект оказался полезен, вы можете поддержать дальнейшую разработку:
