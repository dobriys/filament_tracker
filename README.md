...
- **Журнал ошибок (диагностика)** — включаемая запись ошибок для отладки: при
  проблеме включите запись, повторите действия, скачайте журнал и приложите его
  к issue. См. [Как сообщить об ошибке](#как-сообщить-об-ошибке).
...
1. **Настройки → Журнал ошибок (диагностика)** → включите **«Записывать ошибки
   (бэкенд + браузер)»** (доступно администратору).
2. Повторите действия, на которых возникает проблема.
3. Нажмите **«Скачать журнал (.txt)»** и приложите файл к issue (или скопируйте
   текст из **«Показать журнал»**). Опишите, что делали и что ожидали увидеть.
4. После отправки запись можно выключить, а журнал — **«Очистить»**.

Что попадает в журнал: необработанные ошибки сервера (тип, сообщение,
трассировка, метод и путь запроса) и ошибки в браузере. Журнал хранится **только
в памяти сервера** (последние 500 записей) и стирается при перезапуске — во
внешние сервисы ничего не отправляется. Личных данных в трассировках, как
правило, нет, но перед публикацией просмотрите файл.
